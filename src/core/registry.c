...
{return ng_level_generation_policy_version(id,difficulty,mode,ng_pack_revision(id,mode));}
unsigned ng_level_generation_policy_version(unsigned id,unsigned difficulty,unsigned mode,unsigned revision)
{
 if(id==6 && revision<=2)return NG_SUPPLY_BANK;
 if(id==19)return mode==1?(revision<=2?NG_SUPPLY_TRANSFORMS:NG_SUPPLY_RULES):NG_SUPPLY_TRANSFORMS;
 if(ng_bank_count_version(id,difficulty,mode,revision))return NG_SUPPLY_BANK;
...
void ng_new_supply(NgGame *g,unsigned id,unsigned difficulty,unsigned mode,uint32_t seed,uint32_t run_id,uint32_t supply_seed,uint32_t supply_index)
{
 ...
 ng_new_supply_version(g,id,difficulty,mode,seed,run_id,supply_seed,supply_index,ng_pack_revision(id,mode));
}
void ng_new_supply_version(NgGame *g,unsigned id,unsigned difficulty,unsigned mode,uint32_t seed,uint32_t run_id,uint32_t supply_seed,uint32_t supply_index,unsigned revision)
{
 ...
 g->generation_policy=ng_level_generation_policy_version(g->id,g->difficulty,policy_mode,revision);
...
static bool valid(const NgGame *g)
{
 ...
 unsigned policy=ng_level_generation_policy_version(g->id,g->difficulty,policy_mode,g->pack_revision);
