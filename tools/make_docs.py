...
games=catalog['games'];assert [g['id'] for g in games]==list(range(1,29))+[31,32]
...
for g in games:
    flag=g['flags'];hint='REVEAL' if 11<=g['id']<=20 and flag&2 else 'HINT' if flag&2 else '—'
    extra='; ANSWER' if g['aux']=='ANSWER' else '; REVEAL' if g['aux']=='REVEAL' else ''
    modes=' / '.join(g['modes'])
    rows.append(f"| {g['id']:02} | {g['name']} | {modes} | {'Yes / Yes' if flag&4 else '—'} | {'Yes' if flag&1 else '—'} | {hint}{extra} | HOST VERIFIED · TARGET BUILT · HARDWARE PENDING |")
...
print('Exported 30 actual registry entries, rules and stage matrix')
