...
int main(void)
{
 ...
 for(unsigned index=0;index<NG_GAME_COUNT;index++){
  ...
  printf(",\"category\":%u,\"slot\":%u,\"difficulties\":%u,\"regular_difficulties\":%u,\"has_hell\":%s,\"generation_policy\":%u,\"flags\":%u,\"aux\":",index/6,index%6,ng_difficulty_count(id),ng_regular_difficulty_count(id),ng_has_hell(id)?"true":"false",ng_generation_policy(id),m->flags);quote(m->aux_label);fputs(",\"primary\":",stdout);quote(m->primary_label);
