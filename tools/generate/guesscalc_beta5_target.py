#!/usr/bin/env python3
"""Revision5 bounded-card target-indexed bank: 1000 targets × four levels × two records.

Generation is deterministic and bounded to 4096 candidate indices per cell and every card is 1..999.
...
JSON = ROOT/'assets/guesscalc_target_beta5.json'
HEADER = ROOT/'assets/guesscalc_target_beta5.h'
CSV = ROOT/'assets/guesscalc_target_beta5_complexity.csv'
LEVELS = ('EASY', 'NORMAL', 'HARD', 'MASTER')
MASTER_BETA4_MIN_SCORE = 4_472_963
_solver = None
...
def accepted(d, result):
    ...
    # Three-digit cards may change the exact minimum, but MASTER must retain
    # at least beta.4's minimum published structural-plus-rarity score.
    rarity = max(0, 16-(result['canonical_count'].bit_length()-1))
    return g['fractional_steps'] >= 1 and 32*g['score']+rarity >= MASTER_BETA4_MIN_SCORE
...
def make_target(target):
    ...
    for difficulty in range(4):
        ...
        for index in range(1, 4097):
            ...
            if not cards:
                continue
            assert len(witness) < 40 and all(1 <= v <= 999 for v in cards)
            ...
            if not accepted(difficulty, result):
                fail += 1
                continue
            made.append(dict(puzzle_id=8240+(difficulty*1000+target-1)*2+len(made), difficulty=difficulty,
...
def verify_target(records):
    for record in records:
        ...
        cards, witness, _ = target_deck_v5(index, d, target)
...
def header(data):
    ...
    return '\n'.join(['/* Revision5 bounded-card exact-graded indices. Host metadata is not linked. */',
                      '#ifndef GUESSCALC_TARGET_BETA5_H', '#define GUESSCALC_TARGET_BETA5_H',
                      'static const unsigned short gc_target_beta5_index[8000]={', *rows, '};', '#endif', ''])
...
def validate(data):
    assert data['revision'] == 5 and data['targets'] == 1000 and data['per_target_level'] == 2
    ...
    for ordinal, r in enumerate(data['records']):
        ...
        assert (r['puzzle_id'], r['difficulty'], r['target']) == (8240+ordinal, d, target)
        cards, witness, _ = target_deck_v5(r['candidate_index'], d, target)
        assert cards and all(1 <= v <= 999 for v in cards)
...
def main():
    ...
    if args.generate:
        ...
        data = dict(revision=5, targets=1000, per_target_level=2, external_records=0, construction_version='beta5-bounded-xorshift-v1',
