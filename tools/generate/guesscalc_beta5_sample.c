/* Host-only C/JSON parity probe for every revision5 target record. */
#include "ng.h"
#include "guesscalc.h"
...
int main(int argc,char **argv){
 ...
 for(unsigned d=0;d<4;d++)for(unsigned target=1;target<=1000;target++)for(unsigned ordinal=0;ordinal<2;ordinal++){
  NgGame g={0};g.id=6;g.difficulty=(uint8_t)d;g.seed=g.rng=739;g.run_id=1;g.pack_revision=5;
  ...
  for(unsigned i=0;i<(unsigned)g.data[1];i++)assert(g.board[i]>=1&&g.board[i]<=999);
