...
beta.4에서는 새 Countdown HARD 문제의 1~3장 지름길을 없애고, Sequence EASY/NORMAL의 규칙 계열을 구분했습니다. Prime Factor MASTER의 소인수 구조를 강화했고 Sliding EASY/NORMAL/HARD는 최단 이동 횟수를 검증한 보드를 사용합니다. Make Target은 PC의 전수 풀이기로 가장 쉬운 합법 해법을 확인한 문제를 목표 숫자별로 제공하며, 새 MASTER 문제는 중간 분수 계산이 필요합니다. beta.5에서는 이를 **모든 카드 1~999**로 다시 생성하고, 긴 RULES에만 스크롤 위치 표시를 넣었습니다. Sequence 여섯 카드의 읽기 순서와 Prime Factor 목표 숫자의 중앙 정렬도 개선했습니다. 진행 중이던 예전 게임의 문제와 규칙은 유지됩니다. [카드 크기 감사](docs/MAKE_TARGET_READABILITY_AUDIT.md)와 [36게임 화면 정렬 감사](docs/UI_ALIGNMENT_AUDIT.md)에 측정값과 캡처가 있습니다.
