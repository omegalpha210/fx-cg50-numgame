...
Beta.4 removes 1–3-card exact shortcuts from new Countdown HARD games, gives Sequence EASY and NORMAL different rule-family mixes, strengthens Prime Factor MASTER, and uses certified minimum move counts for new Sliding EASY/NORMAL/HARD boards. Make Target selects each target's decks using the easiest legal expression found by a host exhaustive solver; new MASTER decks require fractional intermediate arithmetic. Beta.5 regenerates these decks with every card limited to **1–999**, adds a scroll position indicator only to long RULES, makes Sequence's six-card reading order explicit, and centers Prime Factor targets using measured font widths. Existing unfinished games keep their original puzzles and rules. See the [card readability](docs/MAKE_TARGET_READABILITY_AUDIT.md) and [36-game UI alignment](docs/UI_ALIGNMENT_AUDIT.md) audits.
