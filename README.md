...
Navigate the two-by-three main and category grids with arrows or 1–6; EXE/F6 OPEN selects a tile. Before a game, use UP/DOWN to select NEW GAME, a saved game, difficulty, or an applicable setting. LEFT/RIGHT changes the selected setting and the footer explains the row. F6 OPEN performs that row's action; F5 opens RULES. There is no STATS screen or separate MODE popup. F3 toggles LOGIC HELL and shows a colored ENHM key when selected. Strategy FIRST selects YOU or CPU.
...
Only the **five most recently played games** retain resumable saves. Starting a sixth game asks before deleting the oldest save. Two copies of each retained record are created at their exact encoded size; startup no longer preallocates two 462,032-byte archives. Existing `NGARCA.dat`/`NGARCB.dat` saves migrate at startup. Old records lack exact last-play times, so migration retains the last game first, then the highest save generations up to five. Aggregate statistics are not carried into the compact format. See [storage format](docs/STORAGE_FORMAT.md) and [migration/recovery](docs/STORAGE_ARCHIVE_AUDIT.md).
